"""Train the worm segmentation model with Ultralytics YOLO26m-seg."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train worm segmentation with YOLO26m-seg.")
    parser.add_argument(
        "--weights",
        default="models/pretrained/yolo26m-seg.pt",
        help="YOLO26m-seg pretrained weights or model name. 'yolov26m-seg' is accepted as an alias.",
    )
    parser.add_argument(
        "--data",
        default="label/hard_finetune_dataset/dataset.yaml",
        help="YOLO segmentation dataset.yaml",
    )
    parser.add_argument("--project", default="worm_yolo_results", help="Training output root")
    parser.add_argument("--name", default="train_yolo26m_seg", help="Training run name")
    parser.add_argument("--epochs", type=int, default=150, help="Maximum training epochs")
    parser.add_argument("--patience", type=int, default=50, help="Early stopping patience")
    parser.add_argument("--batch", type=int, default=1, help="Batch size. Use 1 for YOLO26m-seg on 8GB GPUs.")
    ...
    parser.add_argument(
        "--optimizer",
        default="auto",
        help="Optimizer. Default 'auto' lets Ultralytics choose optimizer and LR for initial YOLO26 training.",
    )
    parser.add_argument(
        "--lr0",
        type=float,
        default=None,
        help="Initial LR. Only passed when --optimizer is not 'auto'.",
    )
    ...
    if version_tuple(ultralytics.__version__) < MIN_ULTRALYTICS:
        raise RuntimeError(
            ...
            f"current version is {ultralytics.__version__}. Run: python -m pip install -U 'ultralytics>=8.4.52'"
        )
    ...
    if args.optimizer.lower() != "auto" and args.lr0 is not None:
        ...
        print("Note: --optimizer auto ignores --lr0; omit --lr0 or choose --optimizer AdamW/SGD to force it.")
